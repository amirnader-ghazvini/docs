...
The common missconception is that the FXTL points are mapped at a one-to-one ratio with frxUSD, which is not the case.
The base earning rate is derived using the market conditions dinamically and fluctuates over time.
...
Some of the Farms are more complex than others. While of the farms all use the effective balance to calculate the FXTL
points earned, some use multiple sources to determine users' balances (example of these are BAMM, Curve, and Karak
farms).
